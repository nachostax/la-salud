  ...
  return `
    <div class="wo-pl-row wo-pl-warmup ${row.completed ? 'done' : ''} ${isCurrent ? 'wo-pl-row-current' : ''}">
      ...
      <div class="wo-pl-hold-btn ${row.completed ? 'checked' : ''}"
        ...
        ${row.completed
          ? '<svg class="wo-pl-hold-icon" viewBox="0 0 24 24" width="16" height="16"><path fill="currentColor" d="M9 16.2 4.8 12l-1.4 1.4L9 19 21 7l-1.4-1.4z"/></svg>'
          : '<svg class="wo-pl-hold-icon" viewBox="0 0 24 24" width="16" height="16"><circle cx="12" cy="12" r="9" fill="none" stroke="currentColor" stroke-width="2"/></svg>'}
      </div>
    </div>`;
}
// Full-screen glow used only while the warmup hold is in progress — fades
// in over the same 3s as the hold itself (like gaining life in a game),
// then fades straight back out the instant it's cancelled or completes.
// It's a single reused fixed element appended to <body> once, so normal
// re-renders of the row list (renderPlayerBody) never recreate or disturb
// it mid-hold.
...
function startWarmupHold(i) {
  ...
  _warmupHoldTimer = setTimeout(() => {
    ...
    // Brief flash at full brightness, then fade back to normal — after
    // this the screen looks exactly as it did before the hold started.
...
function cancelWarmupHold(source) {
  ...
  if (glow) { glow.style.transition = 'opacity .25s ease'; glow.style.opacity = '0'; }
}
