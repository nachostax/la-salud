...
function getWorkoutStyle(w) {
  const circuitCount = w.entries.filter(e => e.type === 'set' && e.sets === 1 && e.restSeconds === 0).length;
  if (circuitCount >= 3) return 'HIIT';
  const counts = {};
  w.entries.forEach(e => {
    const ex = getExerciseById(e.exerciseId);
    if (ex) counts[ex.style] = (counts[ex.style] || 0) + 1;
  });
...
function refreshWorkoutList() {
  const el = document.getElementById('wo-list-body');
  if (el) el.innerHTML = renderWorkoutListRows(wmState());
}
  ...
  if (log.length === 0) {
    return `
      <div class="tab-lock-box" style="margin:32px auto 0">
        ...
        <div class="tab-lock-body">Finish a workout from the New Workout tab and your history and progress charts will show up here.</div>
      </div>`;
  }
