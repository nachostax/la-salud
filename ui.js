...
function restoreFromCSV(event) {
  ...
  reader.onload = e => {
    ...
    for (let i=1;i<lines.length;i++) {
      ...
      existing.add(sig);
      if (rtype === 'WORKOUT') {
        S.entries.push({ id:Date.now()+Math.random(), record_type:'workout', person, date, workout_type:name, logged_at:time, duration_min:parseFloat(duration_min)||0, intensity, calories_burned:parseFloat(calories_burned)||0, steps_logged:parseFloat(steps_logged)||0, notes });
      } else if (rtype === 'WATER') {
