    ...
    <div class="mission-block visible" data-person="${p}">
      ...
      <div class="mfield">
        ...
        <div style="font-family:'Baloo 2',sans-serif;font-size:10px;color:var(--mist);margin-top:4px">
          Recomposition uses your weight target below like every other goal — set it equal to your current weight to hold steady while training reshapes composition, or set a real target if you also want to lose or gain.
        </div>
      </div>
...
// ── QUICK LOG EDITS ────────────────────────────────────────────────────────
// AI-capture mode for the Vitamins quick-log editor — UI-only state, not
// persisted (always opens on "AI" since that's now the primary path; the
// numeric fields below stay identical either way, "Manual" just hides the
// text/photo capture block above them).
let qlVitMode = 'ai';
let qlVitPhotos = [];   // [{id, data, mime}] — same shape as autoPhotos in ui.js
let qlVitPhotoSeq = 0;

function setQlVitMode(mode) {
  qlVitMode = mode;
  const panel = document.getElementById('ql-vit-ai-panel');
  if (panel) panel.style.display = mode === 'ai' ? 'block' : 'none';
  const pillAi = document.getElementById('ql-vit-mode-ai');
  const pillManual = document.getElementById('ql-vit-mode-manual');
  if (pillAi && pillManual) {
    pillAi.classList.toggle('active', mode === 'ai');
    pillManual.classList.toggle('active', mode === 'manual');
  }
}

function handleQlVitPhotos(event) {
  const files = [...event.target.files];
  const prev = document.getElementById('ql-vit-photo-preview');
  files.forEach(file => {
    const id = ++qlVitPhotoSeq;
    const reader = new FileReader();
    reader.onload = e => {
      qlVitPhotos.push({ id, data: e.target.result.split(',')[1], mime: file.type });
      if (!prev) return;
      ...
      del.textContent = '✕';
      del.onclick = () => removeQlVitPhoto(id);
...
function removeQlVitPhoto(id) {
  qlVitPhotos = qlVitPhotos.filter(p => p.id !== id);
  const el = document.querySelector('#ql-vit-photo-preview .photo-thumb[data-photo-id="' + id + '"]');
  if (el) el.remove();
}

// Builds the prompt asking the AI to read a supplement/vitamin stack
// (photo of label(s) and/or free-text description) and return ONE
// pipe-delimited line covering every micro this app tracks. If several
// supplements are described/photographed together as one daily stack,
// the AI is told to sum them into one combined total.
function vitaminsAIPrompt(desc) {
  return `You are reading nutrition-facts data for a supplement/vitamin stack, to save as a reusable "quick log" preset in a personal health-tracking app.
${desc ? 'Description from the person: ' + desc : 'Read the nutrition facts from the attached photo(s) of the supplement label(s)/bottle(s).'}

If multiple supplements are described or photographed together (e.g. a magnesium capsule + a vitamin D drop + a B12 tablet, all taken as one daily stack), SUM their values into one combined total — this preset represents everything taken in one sitting.

Reply with EXACTLY one line, no other text, no markdown, in this exact pipe-delimited format. Use 0 for any nutrient not present or not stated — never omit a field:
VITAMINS|Meal: <short label, e.g. "Vitamins">|Calories: <kcal>|Protein_g: <g>|Carbs_g: <g>|Fat_g: <g>|Fibre_g: <g>|Magnesium_mg: <mg>|Vitd_mcg: <mcg>|Iron_mg: <mg>|Calcium_mg: <mg>|Zinc_mg: <mg>|B12_mcg: <mcg>|Omega3_g: <g>|Potassium_mg: <mg>|Vitc_mg: <mg>|Folate_mcg: <mcg>`;
}

// Pushes AI-read values into the (always-present) manual fields below —
// same fields Save Vitamins already reads from, so nothing else changes.
function fillQuickLogVitaminFields(raw) {
  const set = (id, v) => { const el = document.getElementById(id); if (el && v !== undefined) el.value = v; };
  if (raw.meal) set('ql-vit-name', raw.meal);
  set('ql-vit-cal', raw.calories);
  set('ql-vit-carbs', raw.carbs_g);
  set('ql-vit-protein', raw.protein_g);
  set('ql-vit-fat', raw.fat_g);
  set('ql-vit-fibre', raw.fibre_g);
  set('ql-vit-mag', raw.magnesium_mg);
  set('ql-vit-vitd', raw.vitd_mcg);
  set('ql-vit-iron', raw.iron_mg);
  set('ql-vit-calcium', raw.calcium_mg);
  set('ql-vit-zinc', raw.zinc_mg);
  set('ql-vit-b12', raw.b12_mcg);
  set('ql-vit-omega3', raw.omega3_g);
  set('ql-vit-potassium', raw.potassium_mg);
  set('ql-vit-vitc', raw.vitc_mg);
  set('ql-vit-folate', raw.folate_mcg);
}

async function analyzeQuickLogVitaminsAI() {
  const descEl = document.getElementById('ql-vit-ai-desc');
  const desc = descEl ? descEl.value.trim() : '';
  const hasPhoto = qlVitPhotos.length > 0;
  if (!desc && !hasPhoto) { showToast('Describe the supplement(s) or add a photo first'); return; }

  const hasAnyKey = getGeminiKeys().length || getGroqKeys().length || getCerebrasKeys().length;
  if (!hasAnyKey) { showToast('Add an AI API key in Settings first'); return; }
  if (hasPhoto && !getGeminiKeys().length) {
    showToast('Photo reading needs a Gemini key (Groq/Cerebras are text-only). Add one in Settings, or describe it in words instead.');
    return;
  }

  const btn = document.getElementById('ql-vit-ai-btn');
  setBtnThinking(btn, true, 'Reading…');
  try {
    const promptText = vitaminsAIPrompt(desc);
    let text;
    if (hasPhoto) {
      const imageParts = qlVitPhotos.map(p => ({ inline_data: { mime_type: p.mime, data: p.data } }));
      const attempts = [];
      text = null;
    ...
    const line = text.split('\n').map(l => l.trim()).find(l => /^VITAMINS/i.test(l));
    if (!line) throw new Error('AI reply unreadable — try again or use Manual mode');
    const raw = normaliseLine(line);
    fillQuickLogVitaminFields(raw);
    // Clear the capture inputs — the read values now live in the fields below.
    if (descEl) descEl.value = '';
    const preview = document.getElementById('ql-vit-photo-preview');
    if (preview) preview.innerHTML = '';
    qlVitPhotos = [];
    showToast('Read ✓ — check the values below, then Save vitamins');
  } catch (e) {
    showToast(e.message || 'Could not reach AI');
  } finally {
    setBtnThinking(btn, false, 'Analyze with AI');
  }
}
  ...
  document.getElementById('quicklog-body').innerHTML = `
    ...
    <div class="trend-card">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px">
        <div style="font-family:'Baloo 2',sans-serif;font-style:italic;font-size:15px">💊 ${p === 'gabi' ? 'Gabi' : 'Nacho'}'s vitamins</div>
        <div class="seg pf-toggle" id="ql-vit-mode-toggle">
          <span class="seg-opt pf-toggle-option ${qlVitMode === 'ai' ? 'active' : ''}" id="ql-vit-mode-ai" onclick="setQlVitMode('ai')">AI</span>
          <span class="seg-opt pf-toggle-option ${qlVitMode === 'manual' ? 'active' : ''}" id="ql-vit-mode-manual" onclick="setQlVitMode('manual')">Manual</span>
        </div>
      </div>

      <!-- AI CAPTURE: describe/photograph the supplement(s), AI fills the
           fields below — same idea as the Log tab's Auto meal logger. -->
      <div id="ql-vit-ai-panel" style="display:${qlVitMode === 'ai' ? 'block' : 'none'};margin-bottom:12px">
        <label class="file-btn" style="margin-bottom:10px">
          Choose photo(s)
          <input type="file" accept="image/*" multiple id="ql-vit-photo-input" onchange="handleQlVitPhotos(event)">
        </label>
        <div id="ql-vit-photo-preview" style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:10px"></div>
        <textarea id="ql-vit-ai-desc" placeholder="e.g. 'Magnesium glycinate 400mg + vitamin D3 4000iu + B12 1000mcg'" style="min-height:44px;width:100%"></textarea>
        <button class="btn btn-secondary" id="ql-vit-ai-btn" style="width:100%;margin-top:8px" onclick="analyzeQuickLogVitaminsAI()">Analyze with AI</button>
      </div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px">
        ...
        <div class="mfield" style="margin:0"><label style="font-size:10px">fat g</label><input type="number" id="ql-vit-fat" value="${(s.quickLogOverrides?.[p]?.vitamins?.fat_g) ?? vitamins[p]?.fat_g ?? 0}" step="0.1" style="margin:0"></div>
        <div class="mfield" style="margin:0"><label style="font-size:10px">fibre g</label><input type="number" id="ql-vit-fibre" value="${(s.quickLogOverrides?.[p]?.vitamins?.fibre_g) ?? vitamins[p]?.fibre_g ?? 0}" step="0.1" style="margin:0"></div>
        ...
        <div class="mfield" style="margin:0"><label style="font-size:10px">iron mg</label><input type="number" id="ql-vit-iron" value="${(s.quickLogOverrides?.[p]?.vitamins?.iron_mg) ?? vitamins[p]?.iron_mg ?? 0}" step="0.1" style="margin:0"></div>
        <div class="mfield" style="margin:0"><label style="font-size:10px">calcium mg</label><input type="number" id="ql-vit-calcium" value="${(s.quickLogOverrides?.[p]?.vitamins?.calcium_mg) ?? vitamins[p]?.calcium_mg ?? 0}" style="margin:0"></div>
        <div class="mfield" style="margin:0"><label style="font-size:10px">zinc mg</label><input type="number" id="ql-vit-zinc" value="${(s.quickLogOverrides?.[p]?.vitamins?.zinc_mg) ?? vitamins[p]?.zinc_mg ?? 1.5}" step="0.1" style="margin:0"></div>
        ...
        <div class="mfield" style="margin:0"><label style="font-size:10px">omega-3 g</label><input type="number" id="ql-vit-omega3" value="${(s.quickLogOverrides?.[p]?.vitamins?.omega3_g) ?? vitamins[p]?.omega3_g ?? 0}" step="0.1" style="margin:0"></div>
        <div class="mfield" style="margin:0"><label style="font-size:10px">potassium mg</label><input type="number" id="ql-vit-potassium" value="${(s.quickLogOverrides?.[p]?.vitamins?.potassium_mg) ?? vitamins[p]?.potassium_mg ?? 0}" style="margin:0"></div>
        <div class="mfield" style="margin:0"><label style="font-size:10px">vit C mg</label><input type="number" id="ql-vit-vitc" value="${(s.quickLogOverrides?.[p]?.vitamins?.vitc_mg) ?? vitamins[p]?.vitc_mg ?? 12}" step="0.1" style="margin:0"></div>
        <div class="mfield" style="margin:0"><label style="font-size:10px">folate mcg</label><input type="number" id="ql-vit-folate" value="${(s.quickLogOverrides?.[p]?.vitamins?.folate_mcg) ?? vitamins[p]?.folate_mcg ?? 83.3}" step="0.1" style="margin:0"></div>
      </div>
...
function saveQuickLogVitamins() {
  ...
  const cal       = num('ql-vit-cal', 18);
  const carbs     = num('ql-vit-carbs', 4.3);
  const protein   = num('ql-vit-protein', 0.1);
  const fat       = num('ql-vit-fat', 0);
  const fibre     = num('ql-vit-fibre', 0);
  const mag       = num('ql-vit-mag', 175);
  const vitd      = num('ql-vit-vitd', 2.1);
  const iron      = num('ql-vit-iron', 0);
  const calcium   = num('ql-vit-calcium', 0);
  const zinc      = num('ql-vit-zinc', 1.5);
  const b12       = num('ql-vit-b12', 2.2);
  const omega3    = num('ql-vit-omega3', 0);
  const potassium = num('ql-vit-potassium', 0);
  const vitc      = num('ql-vit-vitc', 12);
  const folate    = num('ql-vit-folate', 83.3);
  const vitOverride = {
    meal: name,
    calories: cal,
    protein_g: protein,
    ...
    zinc_mg: zinc, b12_mcg: b12,
    omega3_g: omega3, potassium_mg: potassium, vitc_mg: vitc, folate_mcg: folate
  };
...
function renderApiKeyBody() {
  ...
  const homeP = homePerson();
  document.getElementById('apikey-body').innerHTML = `
    <div class="trend-card">
      <div style="font-family:'Baloo 2',sans-serif;font-style:italic;font-size:15px;margin-bottom:10px">This device</div>
      <div style="font-family:'Baloo 2',sans-serif;font-size:10px;color:var(--mist);margin-bottom:10px">
        Which person this phone opens to by default. Only affects this device — doesn't touch shared data.
      </div>
      <div style="display:flex;gap:8px">
        <button type="button" class="btn ${homeP==='gabi'?'btn-primary':'btn-secondary'}" style="flex:1" onclick="setHomePerson('gabi')">Gabi's phone</button>
        <button type="button" class="btn ${homeP==='nacho'?'btn-primary':'btn-secondary'}" style="flex:1" onclick="setHomePerson('nacho')">Nacho's phone</button>
      </div>
    </div>
    <div class="trend-card">
...
// Sets which person THIS PHONE defaults to on load (see homePerson()/
// HOME_PERSON_KEY in ui.js). Device-local only — never synced. Also
// switches the currently-viewed person immediately so the toggle feels
// like it did something, rather than only taking effect on next launch.
function setHomePerson(p) {
  try { localStorage.setItem(HOME_PERSON_KEY, p); } catch (e) {}
  renderApiKeyBody();
  if (S.currentPerson !== p) setPerson(p);
  showToast(`This phone now opens to ${p === 'gabi' ? 'Gabi' : 'Nacho'} ✓`);
}
