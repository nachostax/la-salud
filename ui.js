...
// instant). Two flavors:
//   'meal'    → EATINGGIF.gif, "Ñam Ñam" / "Digesting your delicious data"
...
const DIGEST_COPY = {
  meal: {
    gif: 'https://raw.githubusercontent.com/nachostax/la-salud2/main/EATINGGIF.gif',
    title: 'Ñam Ñam',
...
function preloadDigestGifs() {
  Object.values(DIGEST_COPY).forEach(copy => {
    if (_digestPreloaded[copy.gif]) return;
    _digestPreloaded[copy.gif] = true;
    const img = new Image();
    img.src = copy.gif;
  });
}
...
function showDigestOverlay(kind) {
  ...
  imgEl.onload = () => { if (seq === digestOverlaySeq) imgEl.style.opacity = '1'; };
  imgEl.onerror = () => { if (seq === digestOverlaySeq) imgEl.style.opacity = '1'; };
  imgEl.src = copy.gif;
