...
// ── SKILL-LADDER MECHANISM ──────────────────────────────────────────────
// Backs `{type:'skill', progressionGroup, ...}` workout entries (see
// workout-library.js header). Level is stored per person, per chain, at
// wm.skillLevels[progressionGroup] — an integer matching a `progressionLevel`
// in that chain. Seeded once per chain on first read, never overwritten
// after that by anything but the auto-advance check below.
const SKILL_LEVEL_SEED = { nacho: 3, gabi: 2 };

function getSkillGroup(progressionGroup) {
  return PROGRESSION_GROUPS.find(g => g.id === progressionGroup);
}

// PROGRESSION_GROUPS[i].levels is the source of truth for threshold /
// thresholdUnit / maxLevel — those live on the group's level objects, not
// on the EXERCISE_LIBRARY entries themselves (an exercise only carries
// progressionGroup/progressionLevel back-references).
function getSkillChain(progressionGroup) {
  const group = getSkillGroup(progressionGroup);
  return group ? group.levels.slice().sort((a, b) => a.level - b.level) : [];
}

function getSkillLevel(person, progressionGroup) {
  const wm = S.workoutModule[person];
  if (!wm.skillLevels) wm.skillLevels = {};
  if (wm.skillLevels[progressionGroup] == null) {
    wm.skillLevels[progressionGroup] = SKILL_LEVEL_SEED[person] || 1;
  }
  const group = getSkillGroup(progressionGroup);
  const maxLevel = group ? group.maxLevel : 1;
  return Math.min(wm.skillLevels[progressionGroup], maxLevel);
}

function setSkillLevel(person, progressionGroup, level) {
  const wm = S.workoutModule[person];
  if (!wm.skillLevels) wm.skillLevels = {};
  const group = getSkillGroup(progressionGroup);
  const maxLevel = group ? group.maxLevel : 1;
  wm.skillLevels[progressionGroup] = Math.max(1, Math.min(level, maxLevel));
}

// Turns a `{type:'skill'}` entry into the live exercise for that person's
// current level in the chain — same shape a resolved `'set'` entry would
// have, plus the level bookkeeping (progressionGroup/skillLevel/maxLevel/
// threshold/thresholdUnit) that buildPlayerRows and the auto-advance check
// need downstream. Falls back to the chain's first level rather than
// crashing a session if the stored level somehow has no matching rung.
function resolveSkillEntry(entry, person) {
  const group = getSkillGroup(entry.progressionGroup);
  if (!group || !group.levels.length) return null;
  const level = getSkillLevel(person, entry.progressionGroup);
  const chain = getSkillChain(entry.progressionGroup);
  const lvl = chain.find(l => l.level === level) || chain[0];
  // repsOverride lets an entry (e.g. a max-effort retest) force the shown
  // target to something other than the level's own threshold — e.g. 'mrp'
  // for an all-out AMRAP at whatever exercise the person's level resolves
  // to. The resolved exerciseId/level/threshold bookkeeping is unaffected;
  // only the displayed/logged target label changes.
  const reps = entry.repsOverride
    ? entry.repsOverride
    : (lvl.thresholdUnit === 'seconds' ? `${lvl.threshold}s` : String(lvl.threshold));
  return {
    exerciseId: lvl.exerciseId,
    reps,
    sets: entry.sets,
    restSeconds: entry.restSeconds,
    restAfterExercise: entry.restAfterExercise,
    sideNote: entry.sideNote || null,
    isSkill: true,
    // noLevelAdjust marks a resolved entry (retest/benchmark workouts) as
    // read-only for level purposes — it still shows the person's real
    // current exercise, but neither advances nor demotes the stored level
    // no matter what gets logged. Regular training sessions leave this off.
    noLevelAdjust: !!entry.noLevelAdjust,
    progressionGroup: entry.progressionGroup,
    skillLevel: level,
    maxLevel: group.maxLevel,
    threshold: lvl.threshold,
    thresholdUnit: lvl.thresholdUnit,
    bumpDownThreshold: lvl.bumpDownThreshold
  };
}

// Nudges a single set's level up or down by hand — lives in the input row
// right next to kg, exactly like kg: per-set, always editable, no locking,
// no cascading to other sets of the same exercise. Left untouched, a row
// just shows whatever level the program would normally recommend (the
// value buildPlayerRows resolved it to); pressing +/- re-targets *this row
// only* to a different rung in the chain. This is for the rare case of
// "I know I'm nominally level 1 but today I want to attempt level 2" —
// not a persistent override, just a per-set adjustment. Whether it sticks
// permanently depends only on what's on the *last* set of the exercise when
// it's checked off (see maybeAdjustSkillLevel).
function adjustRowSkillLevel(i, delta) {
  const wm = wmState();
  const row = wm.liveSession.rows[i];
  if (!row.isSkill) return;
  const group = getSkillGroup(row.progressionGroup);
  if (!group) return;
  const newLevel = Math.max(1, Math.min((row.skillLevel || 1) + delta, group.maxLevel));
  if (newLevel === row.skillLevel) return;
  const chain = getSkillChain(row.progressionGroup);
  const lvl = chain.find(l => l.level === newLevel) || chain[0];
  const targetRaw = lvl.thresholdUnit === 'seconds' ? `${lvl.threshold}s` : String(lvl.threshold);
  const classified = classifyRepsTarget(targetRaw);
  row.exerciseId = lvl.exerciseId;
  row.skillLevel = newLevel;
  row.threshold = lvl.threshold;
  row.thresholdUnit = lvl.thresholdUnit;
  row.bumpDownThreshold = lvl.bumpDownThreshold;
  row.targetLabel = classified.label;
  row.repKind = classified.kind;
  row.levelTouched = true;
  save();
  renderPlayerBody();
}

// After the last set of a skill exercise is checked off, compare the best
// logged value that session against the current level's threshold:
//   - meets/beats the advance threshold -> stored level += 1 (capped at max)
//   - falls below the bump-down floor    -> stored level -= 1 (floored at 1)
//   - in between                         -> no change
// Only the LAST set of the exercise triggers this (first/second sets never
// count), and only that set's own logged value and own level matter — each
// set can now independently target a different level via the per-set +/-
// stepper (adjustRowSkillLevel), so "best across all sets" no longer means
// anything coherent once sets can be at different rungs. Whatever level was
// showing on the final set — whether left on the program's recommendation
// or manually adjusted — is what gets locked in as the new stored level.
// noLevelAdjust (retest/benchmark workouts) skips this entirely — those are
// a check-in, not a training session, and never touch the stored level.
function maybeAdjustSkillLevel(i) {
  const wm = wmState();
  const row = wm.liveSession.rows[i];
  if (!row.isSkill || row.noLevelAdjust || !row.completed || row.setNumber !== row.totalSets) return;
  const value = row.thresholdUnit === 'seconds' ? (row.holdSeconds || 0) : (row.reps || 0);
  const person = S.currentPerson || 'gabi';
  if (value >= row.threshold) {
    if (row.skillLevel < row.maxLevel) setSkillLevel(person, row.progressionGroup, row.skillLevel + 1);
  } else if (row.bumpDownThreshold != null && value < row.bumpDownThreshold) {
    if (row.skillLevel > 1) setSkillLevel(person, row.progressionGroup, row.skillLevel - 1);
  }
}
...
// Previously this collapsed to 'Hybrid'/'Full Body' from the *distinct
// count* of categories touched — so a push-dominant session with even one
// incidental core finisher or mobility exercise mixed in got bucketed as
// 'Hybrid' and vanished from the "Push" filter, even though it read as a
// push day. Now it works like getWorkoutStyle: tally entries per category,
// and if one clearly dominates (>=65% of scored entries) the workout is
// tagged with that single pattern. Only genuinely balanced sessions (no
// dominant pattern) fall through to Hybrid/Full Body.
function getWorkoutMuscleGroup(w) {
  ...
  if (!scored) return 'Full Body';
  const touched = Object.keys(counts).length;
  const top = Object.entries(counts).sort((a, b) => b[1] - a[1])[0];
  if (top[1] / scored >= 0.65) return woCap(top[0]);
  if (touched >= 4) return 'Full Body';
  return 'Hybrid';
}
...
// search has text (or a filter is active, or the search bar has been
// engaged) → the flat filtered list, with Style/Muscle/Equipment filters
// visible. `browseListActive` is checked instead of just search-text
// length so that picking a filter with an empty search box still counts
// as "in list mode" — see browseListActive() below for why this needs to
// be persisted state, not a transient flag.
function renderWorkoutBrowseBody(wm) {
  const searchActive = browseListActive(wm);
  ...
  return `
    ...
    <div class="wo-category-back" onclick="exitBrowseSearch()">‹ All Categories</div>
    <div id="wo-list-body">${renderWorkoutListRows(wm)}</div>
  `;
}

// Whether the flat search/filter list should be showing, as opposed to the
// category grid. This used to be driven by the module-level _woSearchFocused
// flag plus search-text length alone. That flag gets forcibly reset to
// false on every renderNewWorkoutPanel() call (see below) — which fires on
// *any* full re-render of the Workout tab, including the app's Firestore
// realtime-sync re-render that runs after every save(). So: focus the
// search bar or tick a filter → toggleFilterValue() calls save() → the
// sync listener fires a global re-render → renderNewWorkoutPanel() zeroes
// _woSearchFocused → and since the search box is still empty (the user
// only used filters, never typed), the view silently snapped back to the
// category grid even though the filters were still selected underneath.
// Fix: persist "the user is in list/filter mode" on wm.browse itself
// (survives re-renders, since it's read fresh from state every time) and
// also treat any active filter as list mode on its own.
function browseListActive(wm) {
  return !!(wm.browse.listViewActive || wm.browse.search.trim().length > 0 ||
    wm.browse.styleFilter.length || wm.browse.muscleFilter.length || wm.browse.equipmentFilter.length);
}

// Leaves the flat filtered list and returns to the category grid,
// clearing search text and all filters so the two views can't disagree
// about what's currently selected.
function exitBrowseSearch() {
  const wm = wmState();
  wm.browse.listViewActive = false;
  wm.browse.search = '';
  wm.browse.styleFilter = [];
  wm.browse.muscleFilter = [];
  wm.browse.equipmentFilter = [];
  wm.browse.openFilter = null;
  save();
...
function onWorkoutSearchFocus() {
  const wm = wmState();
  if (wm.browse.listViewActive) return; // already showing list/filter mode
  wm.browse.listViewActive = true;
  save();
  if (wm.browse.categorySelected) return; // category-grouped view already showing a list
  const body = document.getElementById('wo-browse-body');
  if (body) body.innerHTML = renderWorkoutBrowseBody(wm);
}
...
// A search term matches either the workout's own title, or the name of
// any exercise inside it — so searching "Bulgarian Split Squat" surfaces
// every workout that includes that exercise, even if none of them have
// that phrase in their title. Returns the matched exercise name (or null
// if it matched on title) so the list row can show which exercise pulled
// it in.
function matchWorkoutQuery(w, q) {
  if (!q) return { match: true, exerciseName: null };
  if (w.name.toLowerCase().includes(q)) return { match: true, exerciseName: null };
  for (const e of (w.entries || [])) {
    const ex = getExerciseById(e.exerciseId);
    if (ex && ex.name.toLowerCase().includes(q)) return { match: true, exerciseName: ex.name };
  }
  return { match: false, exerciseName: null };
}

function workoutMatchesFilters(w, wm, tags) {
  const q = wm.browse.search.trim().toLowerCase();
  if (q && !matchWorkoutQuery(w, q).match) return false;
...
function renderWorkoutListRows(wm) {
  ...
  const workoutItems = getAllWorkouts()
    .filter(w => w.standalone !== false)
    .map(w => ({ type: 'workout', w, tags: getWorkoutTags(w), matchedExercise: q ? matchWorkoutQuery(w, q).exerciseName : null }))
  ...
  return items.map(item => {
    ...
    return renderWorkoutListRow(item.w, item.tags, isCustom, item.matchedExercise);
  }).join('');
}

function renderWorkoutListRow(w, tags, isCustom, matchedExercise) {
  return `
    <div class="wo-list-row" style="display:flex;align-items:center;justify-content:space-between;gap:8px" onclick="openWorkoutPreview('${w.id}', false)">
      <div style="min-width:0">
        ...
        ${matchedExercise ? `<div class="wo-list-meta" style="opacity:.8">Includes: ${escapeHtml(matchedExercise)}</div>` : ''}
      </div>
...
function renderProgramListCard(p, wm) {
  ...
  return `
    <div class="wo-program-card" onclick="openProgramPreview('${p.id}')">
      ...
      ${isCurrent
        ...
        : `<button class="btn btn-save" onclick="event.stopPropagation(); enrollProgram('${p.id}')">Enroll</button>`}
    </div>`;
}

// Lets a person see a program's full week-by-week itinerary before
// committing to it, by tapping the card in Available Programs — reuses the
// same overlay/markup as openWorkoutPreview (single #wo-preview-panel) and
// the same row markup as renderFullProgramSchedule, just without the
// done/current status marks since there's no progress to show yet.
function openProgramPreview(programId) {
  const program = getProgramById(programId);
  const panel = document.getElementById('wo-preview-panel');
  if (!program || !panel) return;
  const wm = wmState();
  const isCurrent = wm.activePlanId === program.id;
  const slots = buildProgramSlots(program);
  const totalWeeks = program.weeks.reduce((n, g) => n + g.weekNumbers.length, 0);
  ...
  document.getElementById('wo-preview-inner').innerHTML = `
    <div class="wo-pl-header">
      ...
      <div class="wo-pl-title">${program.name}</div>
      <div style="width:18px;flex-shrink:0"></div>
    </div>
    <div class="wo-preview-body">
      <div class="wo-quick-meta" style="margin:14px 0">${totalWeeks} weeks · ${slots.length} workouts</div>
      <div class="wo-full-sched">
        ${slots.map(s => {
          const w = getWorkoutById(s.workoutIds[0]);
          return `
            <div class="wo-sched-row" onclick="openWorkoutPreview('${s.workoutIds[0]}', false)">
              <span class="wo-sched-week">W${s.weekNumber}</span>
              <span class="wo-sched-name">${w ? w.name : s.workoutIds[0]}</span>
              <span class="wo-sched-status"></span>
            </div>`;
        }).join('')}
      </div>
    </div>
    <div class="wo-pl-footer" style="justify-content:center">
      ${isCurrent
        ? `<div class="status-tag tag-complete">Currently enrolled</div>`
        : `<button class="btn btn-save" onclick="enrollProgram('${program.id}'); closePreviewPanel();">Enroll</button>`}
    </div>`;
...
function renderWorkoutEntriesPreview(workout) {
  ...
  workout.entries.forEach(entry => {
    ...
    if (entry.type === 'skill') {
      const resolved = resolveSkillEntry(entry, S.currentPerson || 'gabi');
      const skillEx = resolved && getExerciseById(resolved.exerciseId);
      const skillName = skillEx ? skillEx.name : (entry.label || entry.progressionGroup);
      html += `<div class="wo-pl-exercise-hdr">${escapeHtml(skillName)}${entry.sideNote ? ' · ' + entry.sideNote : ''}</div>`;
      lastExId = null;
      const classified = resolved ? classifyRepsTarget(resolved.reps) : { label: '' };
...
// Count-up (stopwatch) mode — used for hold-type exercises. Unlike the
// countdown, this doesn't have a fixed duration to run against: it starts
// at 0:00, the SAME pause button by the clock (#wo-bigtimer-pausebtn —
// there is no separate button inside the exercise card) starts it, pauses
// it, and pausing does NOT discard the count — pressing it again resumes
// and keeps adding seconds. Every tick while running writes straight into
// the row's own sec field, so it's always the live number the person can
// also edit by hand. _holdTimerRowIndex tracks which row currently "owns"
// the bar so renderPlayerBody only (re)initializes it when the current
// row changes, not on every re-render.
let _bigTimerMode = 'down';
let _bigTimerElapsed = 0;
let _holdTimerRowIndex = null;
...
// ── SOUND — loud repeating alarm, no external audio file dependency ──
// Gain is pushed to 1.0 (unity — the loudest a WebAudio gain node can go
// without clipping/distorting) rather than the old soft 0.3, on the
// assumption the person will turn their phone volume down themselves if
// they want it quieter, per their own preference, but wants the option to
// go loud. Rings every ~650ms for up to 5s so it can't be missed mid-set,
// but a tap anywhere silences it immediately, like snoozing an alarm.
let _bigTimerAlarmLoop = null;
function stopBigTimerAlarm() {
  if (_bigTimerAlarmLoop) { clearInterval(_bigTimerAlarmLoop); _bigTimerAlarmLoop = null; }
  document.removeEventListener('pointerdown', _bigTimerAlarmTapStop, true);
}
function _bigTimerAlarmTapStop() { stopBigTimerAlarm(); }
function playBell() {
  stopBigTimerAlarm(); // clear any previous loop before starting a fresh one
  const ring = () => {
    try {
      const ctx = new (window.AudioContext || window.webkitAudioContext)();
      const now = ctx.currentTime;
      const tone = (start, freq, dur) => {
      ...
      tone(now, 1046, 0.18);
      tone(now + 0.22, 1318, 0.2);
    } catch (e) { /* audio unavailable — silently skip */ }
  };
  ring();
  document.addEventListener('pointerdown', _bigTimerAlarmTapStop, true);
  let elapsed = 0;
  _bigTimerAlarmLoop = setInterval(() => {
    elapsed += 650;
    if (elapsed >= 5000) { stopBigTimerAlarm(); return; }
    ring();
  }, 650);
}
...
function buildPlayerRows(workout) {
  ...
  workout.entries.forEach(entry => {
    if (entry.type === 'youtube') {
      ...
      const resolved = resolveSkillEntry(entry, S.currentPerson || 'gabi');
      if (!resolved) return;
      const classified = classifyRepsTarget(resolved.reps);
      const totalSets = resolved.sets || 1;
      for (let i = 1; i <= totalSets; i++) {
        rows.push({
          phase: 'main', exerciseId: resolved.exerciseId, kind: 'set',
          setNumber: i, totalSets,
          targetLabel: classified.label, repKind: classified.kind,
          restAfter: resolved.restSeconds || 0,
          restAfterExercise: resolved.restAfterExercise != null ? resolved.restAfterExercise : 90,
          sideNote: resolved.sideNote || null,
          completed: false, reps: null, weight: null, holdSeconds: null, durationMinutes: null,
          isSkill: true, progressionGroup: resolved.progressionGroup,
          bumpDownThreshold: resolved.bumpDownThreshold, noLevelAdjust: resolved.noLevelAdjust,
          skillLevel: resolved.skillLevel, maxLevel: resolved.maxLevel,
          threshold: resolved.threshold, thresholdUnit: resolved.thresholdUnit,
          holdTarget: classified.seconds || null
        });
      }
    } else {
      const classified = classifyRepsTarget(entry.reps);
...
function showBigTimer(opts) {
  ...
  if (opts.seconds == null && !opts.countUp) {
    numEl.textContent = '';
    _bigTimerRemaining = null;
    _bigTimerTotal = null;
  ...
  if (opts.countUp) {
    _bigTimerMode = 'up';
    _bigTimerElapsed = opts.startElapsed || 0;
    _bigTimerPaused = true; // waiting for the person to press the clock's play button
    _bigTimerRemaining = null;
    _bigTimerTotal = null;
    numEl.textContent = fmtClock(_bigTimerElapsed);
    updateBigTimerDonut(0, 0);
    updateBigTimerPauseButton();
    return;
  }
...
function tickBigTimerIfRunning() {
  if (_bigTimerMode === 'up') {
    if (_bigTimerPaused) return;
    _bigTimerElapsed++;
    const numEl = document.getElementById('wo-bigtimer-num');
    if (numEl) numEl.textContent = fmtClock(_bigTimerElapsed);
    const rowIndex = _bigTimerOpts && _bigTimerOpts.rowIndex;
    if (rowIndex != null) {
      const wm = wmState();
      const row = wm.liveSession && wm.liveSession.rows[rowIndex];
      if (row) {
        row.holdSeconds = _bigTimerElapsed;
        const inputEl = document.getElementById(`wo-pl-hold-input-${rowIndex}`);
        if (inputEl) inputEl.value = _bigTimerElapsed;
      }
    }
    return;
  }
...
function toggleBigTimerPause() {
  if (_bigTimerMode === 'up') {
    ...
    if (_bigTimerPaused) save(); // persist the logged seconds once they pause
    updateBigTimerPauseButton();
    return;
  }
...
function updateBigTimerPauseButton() {
  ...
  if (_bigTimerMode === 'up') {
    ...
    btn.setAttribute('aria-label', _bigTimerPaused ? 'Start / resume hold' : 'Pause hold');
    return;
  }
...
function renderPlayerBody() {
  ...
  const live = wm.liveSession;
  if (!live) return;
  // A hold-to-complete gesture (warmup's 3s hold) is a sustained physical
  // press tied to a specific DOM node. If anything — including an
  // externally-triggered re-render from data sync — rebuilds the row list
  // while that press is in progress, the browser tears down the node
  // under the finger/cursor and fires a spurious mouseleave, canceling
  // the hold and resetting it to 0%. Skip the rebuild entirely while a
  // warmup hold is active; the timer's own onDone re-render (or a real
  // cancel) will pick it back up within ~3s at most.
  if (_warmupHoldTimer) return;
  let html = '';
  let lastExId = null;
  let lastPhase = null;
  const firstIncomplete = live.rows.findIndex(r => !r.completed);
  ...
  autoShowHoldTimerIfNeeded(currentIndex);
  ...
  return `
    <div class="wo-pl-row wo-pl-warmup ${row.completed ? 'done' : ''} ${isCurrent ? 'wo-pl-row-current' : ''}">
      <div class="wo-pl-row-main">
        <div class="wo-pl-set-label">Warm up however you like.</div>
        <div class="wo-pl-hold-caption">${row.completed ? 'Warmed up' : 'Done? Hold to confirm.'}</div>
      </div>
      ${row.completed ? '' : '<div class="wo-pl-warmup-tap">TAP</div>'}
      <div class="wo-pl-hold-btn ${row.completed ? 'checked' : ''}" id="wo-pl-warmup-btn-${i}"
        onmousedown="startWarmupHold(${i})" onmouseup="cancelWarmupHold('mouseup')" onmouseleave="cancelWarmupHold('mouseleave')"
        ontouchstart="startWarmupHold(${i})" ontouchend="cancelWarmupHold('touchend')">
        <div class="wo-pl-warmup-fill" id="wo-pl-warmup-fill-${i}"></div>
        <img class="wo-pl-hold-fp wo-pl-hold-fp-white" src="https://raw.githubusercontent.com/nachostax/la-salud2/main/fingerprintwhite.png" alt="" draggable="false">
        <img class="wo-pl-hold-fp wo-pl-hold-fp-black" src="https://raw.githubusercontent.com/nachostax/la-salud2/main/fingerprint.png" alt="" draggable="false">
      </div>
    </div>`;
}
// Full-screen effect used only while the warmup hold is in progress — the
// wash fades in over the same 3s as the hold itself (like gaining life in
// a game), and the rings pulse outward from the center the whole time it's
// active, then both fade straight back out the instant it's cancelled or
// completes. It's a single reused fixed element (with its 3 ring children)
// appended to <body> once, so normal re-renders of the row list
// (renderPlayerBody) never recreate or disturb it mid-hold.
function _woWarmupGlowEl() {
  let el = document.getElementById('wo-warmup-glow');
  if (!el) {
    el = document.createElement('div');
    el.id = 'wo-warmup-glow';
    el.className = 'wo-warmup-glow';
    el.innerHTML = '<div class="wo-warmup-ring"></div><div class="wo-warmup-ring"></div><div class="wo-warmup-ring"></div><div class="wo-warmup-boop"></div>';
...
function startWarmupHold(i) {
  ...
  const btn = document.getElementById('wo-pl-warmup-btn-' + i);
  if (btn) btn.classList.add('pressing');
  const fill = document.getElementById('wo-pl-warmup-fill-' + i);
  if (fill) { fill.style.transition = 'height 3s linear'; requestAnimationFrame(() => fill.style.height = '100%'); }
  const glow = _woWarmupGlowEl();
  glow.classList.remove('boop');
  glow.style.transition = 'none';
  glow.style.opacity = '0';
  glow.classList.add('active');
  requestAnimationFrame(() => {
    glow.style.transition = 'opacity 3s linear';
    glow.style.opacity = '1';
  });
  if (_warmupHoldTimer) clearTimeout(_warmupHoldTimer);
  _warmupHoldTimer = setTimeout(() => {
    _warmupHoldTimer = null;
    wm.liveSession.rows[i].completed = true;
    // "Boop" finish — a fast white flash + shockwave ring burst instead of
    // just fading out, plus a haptic buzz where the device supports it
    // (Android Chrome/Samsung Internet; iOS Safari has no Vibration API,
    // so this is a no-op there — the visual boop still lands either way).
    glow.classList.remove('active');
    glow.classList.add('boop');
    if (navigator.vibrate) navigator.vibrate([35, 25, 90]);
    glow.style.transition = 'opacity .7s ease .2s';
    glow.style.opacity = '0';
    setTimeout(() => glow.classList.remove('boop'), 700);
    save();
    renderPlayerBody();
  }, 3000);
}
function cancelWarmupHold(source) {
  if (_warmupHoldTimer) { clearTimeout(_warmupHoldTimer); _warmupHoldTimer = null; }
  document.querySelectorAll('.wo-pl-hold-btn').forEach(b => b.classList.remove('pressing'));
  document.querySelectorAll('.wo-pl-warmup-fill').forEach(f => { f.style.transition = 'none'; f.style.height = '0%'; });
  const glow = document.getElementById('wo-warmup-glow');
  if (glow) { glow.classList.remove('active', 'boop'); glow.style.transition = 'opacity .25s ease'; glow.style.opacity = '0'; }
}
...
function renderPlayerRow(row, i, ex, name, isCurrent) {
  ...
  const unilateral = !!(ex && ex.unilateral);
  let repsTarget, repsHtml;
  if (row.repKind === 'hold') {
    repsTarget = row.targetLabel;
    const holdLabel = unilateral ? 'sec (each side)' : 'sec';
    repsHtml = `<input type="number" inputmode="numeric" id="wo-pl-hold-input-${i}" class="wo-pl-input" placeholder="${holdLabel}" value="${row.holdSeconds ?? ''}" oninput="playerUpdateField(${i},'holdSeconds',this.value)">`;
    ...
    repsTarget = row.targetLabel;
    const repsLabel = unilateral ? 'reps (each side)' : 'reps';
    repsHtml = `<input type="number" inputmode="numeric" class="wo-pl-input" placeholder="${repsLabel}" value="${row.reps ?? ''}" oninput="playerUpdateField(${i},'reps',this.value)">`;
  }
  // Skill rows show the level stepper in place of kg — kg/assist-kg on a
  // skill exercise refers to added load at a level this person likely isn't
  // even training right now (weighted pistol squat, one-arm push-up, etc.),
  // so it's not a meaningful input here and was just cluttering the row.
  const isSkillLevelRow = row.isSkill && !row.noLevelAdjust;
  let kgHtml = '';
  if (showWeight && !isSkillLevelRow) {
    kgHtml = `
  ...
  // Level stepper — same input-group slot kg would otherwise occupy, per
  // set, always live. Left alone it just shows whatever level the program
  // resolved this row to already; +/- nudges *this set only* to a
  // different rung. No lock, no cascading to other sets — same as how kg
  // works. noLevelAdjust rows (retest/benchmark check-ins) don't get this
  // — level there is read-only, and they never showed kg to begin with.
  let levelHtml = '';
  if (isSkillLevelRow) {
    const atMin = row.skillLevel <= 1;
    const atMax = row.skillLevel >= row.maxLevel;
    levelHtml = `
      <div class="wo-pl-input-group">
        <div class="wo-pl-input-target">Level</div>
        <div class="wo-pl-input" style="display:flex;align-items:center;justify-content:space-between">
          <span onclick="${atMin ? '' : `adjustRowSkillLevel(${i},-1)`}" style="opacity:${atMin ? 0.3 : 1};cursor:${atMin ? 'default' : 'pointer'}">−</span>
          <span class="${row.levelTouched ? 'wo-pl-level-touched' : ''}">${row.skillLevel}</span>
          <span onclick="${atMax ? '' : `adjustRowSkillLevel(${i},1)`}" style="opacity:${atMax ? 0.3 : 1};cursor:${atMax ? 'default' : 'pointer'}">+</span>
        </div>
      </div>`;
  }
...
function playerToggleRow(i) {
  ...
  wm.liveSession.activeRowIndex = i;
  if (row.kind === 'youtube') { save(); renderPlayerBody(); clearBigTimer(); return; }
  if (row.completed) {
    maybeAdjustSkillLevel(i);
    startRestIfNeeded(i);
  } else {
    clearBigTimer();
  }
...
function startRestIfNeeded(i) {
  ...
  showBigTimer({
    label: 'Rest',
    seconds: restSec,
    onDone: () => { showToast('Rest over — next set!'); renderPlayerBody(); },
    skipLabel: 'Skip Rest',
    onSkip: () => renderPlayerBody()
  });
}
...
// ── HOLD COUNT-UP TIMER ── Auto-shown (not tapped open, no button in the
// card) whenever the current row is a hold-type set — mirrors how the
// rest timer just appears between sets, and is controlled with the same
// #wo-bigtimer-pausebtn by the clock as every other timer. Starts at the
// row's already-logged seconds (0 if none yet) so leaving and coming back
// to the same row resumes rather than restarting. See renderPlayerBody's
// call to autoShowHoldTimerIfNeeded after computing currentIndex, and
// tickBigTimerIfRunning/toggleBigTimerPause above for the counting/pause
// behavior itself.
function showHoldTimerForRow(i) {
  const wm = wmState();
  const row = wm.liveSession.rows[i];
  if (!row) return;
  const ex = getExerciseById(row.exerciseId);
  const name = ex ? ex.name : 'the hold';
  showBigTimer({
    label: `Hold ${name}`,
    sub: row.holdTarget ? `Target: ${row.holdTarget}s` : '',
    countUp: true,
    rowIndex: i,
    startElapsed: row.holdSeconds || 0
  });
}
function autoShowHoldTimerIfNeeded(currentIndex) {
  const wm = wmState();
  const live = wm.liveSession;
  if (!live) return;
  const row = live.rows[currentIndex];
  const isHoldRow = row && row.kind === 'set' && row.repKind === 'hold' && !row.completed;
  if (isHoldRow) {
    // If a rest countdown between sets is actively showing, don't stomp
    // on it just because the *next* set also happens to be a hold — let
    // the rest finish (or get skipped) first. startRestIfNeeded's onDone
    // / onSkip re-render once that happens, which brings us back here
    // with no rest in progress, and the hold timer shows then.
    const restInProgress = _bigTimerMode === 'down' && _bigTimerRemaining != null;
    if (restInProgress) return;
    if (_holdTimerRowIndex !== currentIndex) {
      _holdTimerRowIndex = currentIndex;
      showHoldTimerForRow(currentIndex);
    }
  } else if (_holdTimerRowIndex !== null) {
    // We're leaving a hold row. Only tear down the big timer if it's
    // still the hold's own count-up display — if a rest countdown has
    // already started showing (mode 'down'), leave it running instead
    // of nuking it.
    if (_bigTimerMode === 'up') {
      clearBigTimer();
    }
