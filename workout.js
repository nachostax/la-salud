...
function startWarmupHold(i) {
  const wm = wmState();
  console.log('[warmup] startWarmupHold fired, row', i, 'completed=', wm.liveSession.rows[i].completed, 'staleTimer=', _warmupHoldTimer);
  if (wm.liveSession.rows[i].completed) { console.log('[warmup] already completed, bailing'); return; }
  const fill = document.getElementById('wo-pl-hold-fill-' + i);
  if (fill) { fill.style.transition = 'width 3s linear'; requestAnimationFrame(() => fill.style.width = '100%'); }
  if (_warmupHoldTimer) { console.log('[warmup] WARNING: overwriting an existing timer without clearing it:', _warmupHoldTimer); clearTimeout(_warmupHoldTimer); }
  _warmupHoldTimer = setTimeout(() => {
    console.log('[warmup] timer FIRED, marking row', i, 'complete');
    _warmupHoldTimer = null;
    wm.liveSession.rows[i].completed = true;
    save();
    renderPlayerBody();
  }, 3000);
  console.log('[warmup] scheduled new timer id', _warmupHoldTimer);
}
function cancelWarmupHold(source) {
  console.log('[warmup] cancelWarmupHold fired from', source, '- clearing timer=', _warmupHoldTimer);
