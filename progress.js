...
function makeSVGLine(points, color, width, height, target, yAxisMin, unitMode, fixedRange, valueLabel) {
  ...
  if (points.length === 1) {
    ...
    const svg = `<svg class="mini-chart-svg" viewBox="0 0 ${width} ${height}" preserveAspectRatio="none" data-h="${height}">
      ...
      ${targetLine}
      <circle class="mc-dot" data-date="${p.x}" data-val="${p.y}" data-unit="${valueLabel||''}" data-newseg="0" cx="${cx.toFixed(1)}" cy="${cy.toFixed(1)}" r="1.6" fill="${color}" style="cursor:pointer;stroke:transparent;stroke-width:14;pointer-events:all"/>
    </svg>`;
  ...
  const svg = `<svg class="mini-chart-svg" viewBox="0 0 ${width} ${height}" preserveAspectRatio="none" data-h="${height}">
    ...
    ${coords.map((c,i) => c==null ? '' : `<circle class="mc-dot" data-date="${c.date}" data-val="${c.val}" data-unit="${valueLabel||''}" data-newseg="${(i>0&&coords[i-1]==null)?'1':'0'}" cx="${c.x.toFixed(1)}" cy="${c.y.toFixed(1)}" r="1.3" fill="${color}" style="cursor:pointer;stroke:transparent;stroke-width:14;pointer-events:all"/>`).join('')}
  </svg>`;
  return { svg, labelsHtml };
}
