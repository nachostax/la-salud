...
function renderVitals() {
  ...
  if (fullDates.size) {
    let acc = 0, contributingDays = 0;
    fullDates.forEach(d => {
      const dayMeals = kcalMeals.filter(e => e.date === d);
      const dayTotal = sum(dayMeals, 'calories');
      // Use that day's own stamped target snapshot when present — never
      // today's live m.kcal applied retroactively to a past day whose
      // target may have since changed. Same rule the Potates scorer
      // (see the calorie-adherence pillar above) and Progress's
      // getDailyTargetSeries() already use, so all three stay consistent.
      const snap = dayMeals.find(e => e.day_kcal_target > 0);
      const dayTarget = (snap && snap.day_kcal_target) || m.kcal;
      if (!dayTarget) return;
      acc += (dayTotal - dayTarget);
      contributingDays++;
    });
    avgDelta = contributingDays ? acc / contributingDays : null;
    avgDeltaCount = contributingDays;
  }

  // Hero "avg kcal/day" — aligned with Progress's own Calories card:
  // median over full-day-marked days only (not just "any day with an
  // entry"), so both screens report the same figure for the same
  // underlying data. Kept separate from daysWithEntries/avgMeals
  // above, which the macro donut further down still uses (mean over any
  // logged day, today included once it's the only day) — that stat
  // wasn't part of the reported calorie-adherence mismatch. If no day
  // this period has been marked full yet, this is 0 — same as Progress
  // showing '—' in that case, rather than a partial-day figure that
  // can't be compared against anything.
  const fullDayKcalTotals = [...fullDates]
    .map(d => sum(kcalMeals.filter(e => e.date === d), 'calories'))
    .filter(t => t > 0);
  const medianKcalPerFullDay = fullDayKcalTotals.length ? median(fullDayKcalTotals) : 0;

  // ── hero stat ──
  const heroKcal = S.period === 'day' ? totalKcalForTarget : medianKcalPerFullDay;
