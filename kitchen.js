...
// If the current step carries its own timer, preload it in the header —
// ready, not running — so it can be started whenever the person is
// actually ready, independent of marking the step complete. This applies
// to background steps too, so the person can start a concurrent clock
// (e.g. "let stock simmer") early instead of only at the moment they hit
// Done.
function _ckSetupPendingTimerForCurrentStep() {
  if (!ckSession) return;
  const step = ckSession.steps[ckSession.currentIndex];
  ckSession.pendingTimer = (step && step.timerSeconds > 0)
...
// ── Alarm sound — the actual KITCHENTIMER.mp3 from the repo, looped until
// the person dismisses the alert (Stop / ✕). Raw GitHub URL, not the blob
// viewer page, since that serves HTML rather than the audio file itself.
// The bell/beep/chime options all now just trigger this same clip — the
// chip picker still lets someone go silent, which is the only real branch
// left. Volume is maxed; turn the phone down if it's too loud, since the
// clip itself can't safely be pushed louder than 1.0 without clipping.
const CK_ALARM_SOUND_URL = 'https://raw.githubusercontent.com/nachostax/la-salud2/main/KITCHENTIMER.mp3';
let _ckAlarmAudio = null;
function _ckGetAlarmAudio() {
  if (!_ckAlarmAudio) {
    _ckAlarmAudio = new Audio(CK_ALARM_SOUND_URL);
    _ckAlarmAudio.loop = true;
    _ckAlarmAudio.volume = 1.0;
  }
  return _ckAlarmAudio;
}
function ckStopAlarmSound() {
  if (_ckAlarmAudio) { _ckAlarmAudio.pause(); _ckAlarmAudio.currentTime = 0; }
}
let _ckPreviewStopTimer = null;
function ckPlayAlarm(preview) {
  const alarm = ckSession ? ckSession.alarm : 'bell';
  if (alarm === 'silent') { ckStopAlarmSound(); return; }
  const audio = _ckGetAlarmAudio();
  audio.currentTime = 0;
  audio.play().catch(() => {});
  // Previews (tapping a chip in settings) shouldn't loop forever if the
  // person just wanted to hear it — auto-stop after a few seconds. A real
  // timer completion loops indefinitely until Stop/✕ (ckDismissAlert).
  if (preview) {
    if (_ckPreviewStopTimer) clearTimeout(_ckPreviewStopTimer);
    _ckPreviewStopTimer = setTimeout(ckStopAlarmSound, 4000);
  }
...
function ckAdvanceStep() {
  ...
  // The step just completed: if it had its own pending timer already
  // started by the user (running, or partway through), carry it over into
  // the background bar untouched instead of losing or resetting it; if it
  // was never started, it was just an optional convenience clock — drop it.
  let alreadyStarted = false;
  if (ckSession.pendingTimer) {
    const p = ckSession.pendingTimer;
    if (p.running || p.remaining !== p.totalSeconds) {
      ckSession.timers.push(p);
      alreadyStarted = true;
    }
    ckSession.pendingTimer = null;
  }
  // Background steps run their clock concurrently with the rest of the
  // recipe. If the user already started it themselves (above), leave that
  // clock completely alone. Only auto-start a fresh one here if Done was
  // pressed without ever starting it manually.
  if (finishedStep && finishedStep.background && finishedStep.timerSeconds > 0 && !alreadyStarted) {
