...
function makeSVGLine(points, color, width, height, target, yAxisMin, unitMode, fixedRange, valueLabel) {
  ...
  if (points.length === 1) {
    ...
    const svg = `<svg class="mini-chart-svg" viewBox="0 0 ${width} ${height}" preserveAspectRatio="none" data-h="${height}">
      ...
      ${targetLine}
      <circle class="mc-dot" data-date="${p.x}" data-val="${p.y}" data-unit="${valueLabel||''}" data-newseg="0" cx="${cx.toFixed(1)}" cy="${cy.toFixed(1)}" r="1.6" fill="${color}" style="cursor:pointer;stroke:transparent;stroke-width:6;pointer-events:all"/>
    </svg>`;
  ...
  const svg = `<svg class="mini-chart-svg" viewBox="0 0 ${width} ${height}" preserveAspectRatio="none" data-h="${height}">
    ...
    ${coords.map((c,i) => c==null ? '' : `<circle class="mc-dot" data-date="${c.date}" data-val="${c.val}" data-unit="${valueLabel||''}" data-newseg="${(i>0&&coords[i-1]==null)?'1':'0'}" cx="${c.x.toFixed(1)}" cy="${c.y.toFixed(1)}" r="1.3" fill="${color}" style="cursor:pointer;stroke:transparent;stroke-width:6;pointer-events:all"/>`).join('')}
  </svg>`;
  return { svg, labelsHtml };
}
...
// Rolling trailing N calendar days before today (today itself excluded) —
// the window used everywhere in this app for "week"(7) / "month"(30) /
// "year"(365) style periods. A trailing window regardless of what weekday
// today is — NOT anchored to calendar Monday–Sunday boundaries.
...
// Single source of truth for "calorie adherence over some set of days" —
// used by Progress's Calories trend card, Vitals' hero calorie card +
// donut, the Potates score's calorie pillar, and weekly-report.js's PDF.
// Previously each of these hand-rolled its own version — different date
// ranges, different day-filters, mean vs median, fixed vs variable
// divisor — and they drifted out of sync with each other more than once.
//
// Takes an explicit `dates` array rather than deciding its own range —
// callers pass trailingDates(n) for a live rolling window (Vitals/
// Progress/Potates score), or a fixed calendar week (weekly-report.js,
// which is inherently about a specific Mon–Sun week to download) — the
// averaging math below is identical either way.
//
// TRUE average: a day only counts if it was marked full/complete, and
// both avg and avgTarget divide by however many of the given dates
// actually qualify — NOT a fixed divisor like the date list's length.
// A day that wasn't logged/completed is excluded entirely, not counted
// as a zero.
function getFoodCalorieAdherence(person, dates) {
  ...
  const dayRows = dates.map(d => {
    ...
    const target = (snap && snap.day_kcal_target) || (S.mission[person] && S.mission[person].kcal) || 0;
    const total = dayMeals.reduce((a,b) => a + (b.calories||0), 0);
    return { date: d, total, target, isComplete };
  });

  const loggedDays = dayRows.filter(d => d.isComplete && d.total > 0);
  const avg = loggedDays.length ? Math.round(loggedDays.reduce((a,d) => a + d.total, 0) / loggedDays.length) : 0;
  const avgTarget = loggedDays.length ? Math.round(loggedDays.reduce((a,d) => a + d.target, 0) / loggedDays.length) : null;

  const adherence = (avg && avgTarget) ? getAdherence((avg - avgTarget) / avgTarget * 100, 'abs') : null;

  return { dates, dayRows, avg, avgTarget, loggedDays, adherence };
}
...
function renderTrends() {
  ...
  // ── CALORIE TREND — always the canonical getFoodCalorieAdherence(), same
  // dates array every other card on this screen uses ──
  if (progressCategory === 'food') {
    const fc = getFoodCalorieAdherence(person, dates);
    const avg = fc.avg, avgTarget = fc.avgTarget;
    const targetArr = fc.dayRows.map(d => d.target);
    const completeDays = fc.dayRows.map(d => ({ date: d.date, total: d.total, isComplete: d.isComplete }));
...
function deleteHistoryEntry(id) {
  ...
  if (S.usingSubcollections && window.__firebaseSync) {
    ...
    deleteDoc(doc(collection(db, 'la-salud', 'sharedData', 'entries'), String(id)))
      ...
      .catch(err => { console.error('[sync] deleteHistoryEntry failed', id, err); _pendingDeleteIds.delete(id); if (_deletedEntry) _pendingDeleteEntryKeys.delete(entryKey(_deletedEntry)); showToast('Delete failed — check connection'); _fetchFromServer(); });
  } else {
    save();
  }
