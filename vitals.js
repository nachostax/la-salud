...
function potatesScoreForPerson(person, dates, calorieDates) {
  ...
  // Calorie pillar's own date list — defaults to `dates` above, but callers
  // scoring a SPECIFIC historical week (weekly-report.js, scoring a fixed
  // past Mon–Sun week rather than "the last N days from today") pass their
  // exact week's dates here instead, so the calorie pillar scores the same
  // week the rest of the report is about.
  calorieDates = calorieDates || dates;
  ...
  // getFoodCalorieAdherence() (progress.js) — same day-inclusion rule (must
  // be marked full_day), same target-sourcing, same true-average-over-
  // complete-days math as the Vitals/Progress display now, so this pillar's
  // totals always match what's shown on screen for the same date range.
  const fc = getFoodCalorieAdherence(person, calorieDates);
  const dayScores = fc.loggedDays.map(d => ({ date: d.date, pct: (d.total - d.target) / d.target }));
  const totKcal = fc.loggedDays.reduce((a,d) => a + d.total, 0);
  const totTarget = fc.loggedDays.reduce((a,d) => a + d.target, 0);
  ...
  dates.forEach(d => {
    dayCache.get(d).filter(e=>e.record_type==='workout'&&
      (e.workout_type==='HIIT'||e.workout_type==='Cardio-HIIT'||
  ...
  const hiitPts = (hiitMin===0&&activeDays.size===0) ? null : Math.min(5,Math.round((hiitSessionsAchieved/hiitSessTarget)*5));
  ...
  // Scaled by total days in the period (not just logged days) so a single
  // logged day out of a full week can't score as if it were representative
  // of the whole period (previously 1 day at 0.5 quality = 5/10 pts).
  const logPts = logDays>0 ? Math.round((logSum/dates.length)*10) : 0;
...
function potatesScoreForWindow(dates, calorieDates) {
  return potatesScoreForPerson(S.currentPerson, dates, calorieDates).total;
}
function potatesWeeklyScore()  { return potatesScoreForWindow(dateRangeFor('week'), trailingDates(7)); }
function potatesMonthlyScore() { return potatesScoreForWindow(dateRangeFor('month'), trailingDates(30)); }
...
function renderLowsPanel(person, date) {
  ...
  return `<div class="wk-block">
    <div class="meals-label" style="font-family:'Baloo 2',sans-serif;font-size:10px;letter-spacing:2px;color:var(--terra);text-transform:uppercase;margin-bottom:4px">🩸 Lows today</div>
    ${rows}
  </div>`;
}
...
function renderVitals() {
  ...
  // Calorie adherence — canonical getFoodCalorieAdherence() (progress.js),
  // same trailing rolling window (excludes today) that screen uses, so
  // Vitals and Progress can't independently drift again. One call covers
  // week and month both; no more separate hand-rolled month branch.
  let heroKcalPeriod = 0, avgDelta = null, periodTarget = null, weekAdherence = null;
  if (S.period === 'week' || S.period === 'month') {
    const fc = getFoodCalorieAdherence(person, trailingDates(S.period === 'week' ? 7 : 30));
    weekAdherence = fc;
    heroKcalPeriod = fc.avg;
    periodTarget = fc.avgTarget;
    avgDelta = (fc.avg && fc.avgTarget != null) ? (fc.avg - fc.avgTarget) : null;
  }

  // ── hero stat ──
  const heroKcal = S.period === 'day' ? totalKcalForTarget : heroKcalPeriod;
  const heroLabel = S.period === 'day' ? 'Today' : 'avg kcal / day';
  const isToday = S.period === 'day';
  const todayFull = isToday && fullDates.has(todayStr());

  // ── consumed vs target ──
  // The donut below already visualises consumed-vs-target progress (plus
  // overage), so the separate 2-segment bar that used to live here was
  ...
  // ── +/- kcal line ──
  // (goalTargetWeight - weight) < 0 → trying to lose; > 0 → trying to gain; 0 → maintenance.
  const goalDiffKg = (m.goalTargetWeight != null ? m.goalTargetWeight : m.weight) - m.weight;
  const isLosing   = goalDiffKg < 0;
  const isGaining  = goalDiffKg > 0;
  ...
  // When over target, a small red "+n" badge sits right next to the
  // fraction — a quick, always-red flag of the extra eaten, independent of
  // the semantic (good/bad) statusColor used in the line below.
  const heroNumId = 'hero-kcal-' + person;
  const overBadge = (isToday && isOver && consumed > 0)
    ? `<span style="color:var(--status-red-strong);font-size:13px;font-weight:700;margin-left:6px">+${kcalDiff}</span>`
    : '';
  const heroFraction = isToday
    ? `<span id="${heroNumId}" data-target-num="${Math.round(heroKcal)}" style="color:${heroNumColor};transition:color .4s">0</span><span class="hero-frac-sep">/</span><span class="hero-frac-target">${m.kcal}</span>${overBadge}`
  ...
  const nDaysDonut = isToday ? 1 : (weekAdherence ? (weekAdherence.loggedDays.length || 1) : (daysWithEntries.size || 1));
  const donutMeals = isToday ? meals
    : weekAdherence ? kcalMeals.filter(e => weekAdherence.loggedDays.some(d => d.date === e.date))
    : avgMeals;
  ...
  const displayKcal = isToday ? totalKcal : Math.round(heroKcal);
  // Ring fill tracks progress toward the day's calorie target instead of
  // always maxing out the circle — the unfilled remainder is left in a
  // neutral tone so it's obvious at a glance how much of the target is
  // still open. Going more than 10% over target is shown as a proportional
  // red band around the outside of the ring instead of over-filling the
  // main ring past 100%. Ring math itself lives in computeMacroDonutRing()
  // (ui.js), shared with the Log meal/day-detail donut so the two can't
  // drift apart again.
  const donutTarget = isToday ? (m.kcal || 0) : (periodTarget || m.kcal || 0);
  const donutEmptyColor = S.currentPerson === 'gabi' ? 'var(--gabi-moon-edge)' : 'var(--clay)';
  const { background: donutBg, hasOverage, excessPct } =
    computeMacroDonutRing(displayKcal, protein, carbs, fat, donutTarget, donutEmptyColor);
  const DONUT_SIZE = 104;
  const DONUT_GAP = 4;   // px transparent gap between the main ring and the overage band
  const DONUT_RING = 10; // px thickness of the overage ring
  const DONUT_OUTER = DONUT_SIZE + (DONUT_GAP + DONUT_RING) * 2;
  const donutRingOffset = DONUT_RING;              // gap-ring inset from outer edge
  const donutMainOffset = DONUT_RING + DONUT_GAP;   // main donut inset from outer edge
  const donutOuterRingHtml = hasOverage
    ? `<div style="position:absolute;top:0;left:0;width:${DONUT_OUTER}px;height:${DONUT_OUTER}px;border-radius:50%;background:conic-gradient(var(--status-red-strong) 0% ${excessPct.toFixed(2)}%, transparent ${excessPct.toFixed(2)}% 100%)"></div>
       <div class="donut-gap-ring" style="position:absolute;top:${donutRingOffset}px;left:${donutRingOffset}px;width:${DONUT_SIZE+DONUT_GAP*2}px;height:${DONUT_SIZE+DONUT_GAP*2}px;border-radius:50%"></div>
       ${donutOverageMarkerHtml(excessPct/2, DONUT_SIZE/2+DONUT_GAP+DONUT_RING/2, DONUT_OUTER, 'var(--status-red-strong)')}`
    : '';

  const donutHtml = `
    <div class="donut-block">
      <div class="donut-row">
        <div class="donut-wrap"${hasOverage ? ` style="width:${DONUT_OUTER}px;height:${DONUT_OUTER}px"` : ''}>
          ${donutOuterRingHtml}
          <div class="donut" style="background:${donutBg}${hasOverage ? `;position:absolute;top:${donutMainOffset}px;left:${donutMainOffset}px` : ''}"></div>
          <div class="donut-hole"${hasOverage ? ` style="top:${donutMainOffset+14}px;left:${donutMainOffset+14}px"` : ''}>
        ...
        <div class="donut-legend">
          ...
          <div style="height:1px;background:var(--mist);opacity:0.2;margin:3px 0"></div>
          <div class="legend-item"><div class="legend-dot" style="background:${MACRO_FAT_COLOR}"></div><span class="legend-txt">Fat</span><span class="legend-val">${Math.round(fat)} / ${m.fat||'—'}g</span></div>
        </div>
      </div>
    </div>`;
  ...
  const heroHtml = `
    <div class="day-card">
      ...
      <div class="day-card-body">
        <div style="display:flex;gap:12px;align-items:stretch;min-height:84px;margin-bottom:${daysWithEntries.size === 0 && S.period !== 'day' ? '0' : '8px'}">
          <img src="https://raw.githubusercontent.com/nachostax/la-salud2/main/EATINGFULL.png" alt="" style="width:84px;flex-shrink:0;object-fit:contain;margin-top:-4px">
          <div style="flex:1;min-width:0">
            <div class="box-title" style="margin-bottom:6px">Calories</div>
        ...
        ${daysWithEntries.size === 0 && S.period !== 'day' ? '' : donutHtml}
      </div>
    </div>`;
  ...
  if (workouts.length) {
    ...
    workouts.forEach(w => {
      ...
      else if (t === 'HIIT' || t === 'Cardio-HIIT' || (t === 'Cardio' && w.intensity === 'HIIT')) {
    ...
    wkHtml = `<div class="wk-block">
      <div style="display:flex;gap:12px;align-items:stretch;min-height:84px">
        <img src="https://raw.githubusercontent.com/nachostax/la-salud2/main/RUNNINGFULL.png" alt="" style="width:84px;flex-shrink:0;object-fit:contain;margin-top:-4px">
        <div style="flex:1;min-width:0">
          <div class="box-title" style="margin-bottom:6px">Movement</div>
    ...
    wkHtml = `<div class="wk-block">
      <div style="display:flex;gap:12px;align-items:stretch;min-height:84px">
        ...
        <div style="flex:1;min-width:0;display:flex;align-items:center">
          <div>
            <div class="box-title" style="margin-bottom:6px">Movement</div>
            <div style="color:var(--mist);font-size:13px">No workouts logged in this period yet.</div>
          </div>
        </div>
      </div>
    </div>`;
  ...
  let waterHtml = '';
  {
    ...
    // Capped below 100% so there's always a sliver of headroom above the
    // fill for the wave shape to read as water — at a literal 100% height
    // the wave (offset -6px above the fill top) gets clipped by the
    // glass's overflow:hidden and the top just looks flat.
    const waterFillPct = Math.min(Math.round(waterPct * 100), 92);
    waterHtml = wTotal > 0
      ? `<div class="wk-block">
          <div style="display:flex;gap:12px;align-items:stretch;min-height:84px">
            <img src="https://raw.githubusercontent.com/nachostax/la-salud2/main/WATER.png" alt="" style="width:84px;flex-shrink:0;object-fit:contain">
            <div style="flex:1;min-width:0">
              <div class="box-title" style="margin-bottom:6px">Water</div>
              <div class="subheader">${wHit} of ${wTotal} days hit goal</div>
            </div>
            <div class="wtr-glass">
              <div class="wtr-glass-fill" style="height:${waterFillPct}%">
                <svg class="wtr-glass-wave" viewBox="0 0 40 10" preserveAspectRatio="none"><path d="M0 5 Q5 1 10 5 T20 5 T30 5 T40 5 V10 H0 Z" fill="currentColor"/></svg>
              </div>
            </div>
          </div>
        </div>`
      : `<div class="wk-block">
          <div style="display:flex;gap:12px;align-items:stretch;min-height:84px">
            ...
            <div style="flex:1;min-width:0;display:flex;align-items:center">
              <div>
                <div class="box-title" style="margin-bottom:6px">Water</div>
                <div style="color:var(--mist);font-size:13px">No water data logged in this period yet.</div>
              </div>
            </div>
            <div class="wtr-glass">
              <div class="wtr-glass-fill" style="height:0%">
                <svg class="wtr-glass-wave" viewBox="0 0 40 10" preserveAspectRatio="none"><path d="M0 5 Q5 1 10 5 T20 5 T30 5 T40 5 V10 H0 Z" fill="currentColor"/></svg>
              </div>
            </div>
          </div>
  ...
  document.getElementById('vitals-body').innerHTML = renderWeeklyReportBanner() + renderPotatesHero() + caloriesMicroHtml + wkHtml + waterHtml + renderLowsPanel(person, todayStr());
...
function renderPotatesHero() {
  ...
  // Data coverage — logged days vs. total days in the window. Used to tell
  // apart a genuinely bad score from one that's just missing data.
  const scoreDates = dateRangeFor(isMonth ? 'month' : 'week');
  const bd = potatesScoreForPerson(S.currentPerson, scoreDates, trailingDates(isMonth ? 30 : 7));
  const coverageDays = scoreDates.filter(d => d !== todayStr()).length;
  const coverage = coverageDays > 0 ? bd.logDays / coverageDays : 0;
  ...
  const bdr = (lbl, pts, max, note) => {
    ...
    return '<div class="pbd-row" style="padding-left:40px">' +
      '<div class="pbd-row-top">' +
        '<span class="pbd-lbl">' + lbl + '</span>' +
        '<span class="pbd-note">' + note + '</span>' +
        '<span class="pbd-pts">' + pts + '/' + max + '</span>' +
      '</div>' +
      '<div class="pbd-bar-bg"><div class="deficit-fill pbd-bar-fill" data-target-pct="' + pct + '" style="width:' + (_scoreExpanded ? pct : 0) + '%"></div></div>' +
    '</div>';
  };
  const bdSection = (iconUrl, title) =>
    '<div class="pbd-section subheader" style="margin:8px 0 4px;display:flex;align-items:center;gap:8px"><img src="' + iconUrl + '" alt="" style="width:32px;height:32px;object-fit:contain;filter:brightness(1.3)">' + title + '</div>';
  const bdHtml =
    '<div class="pbd-wrap' + (_scoreExpanded ? ' open' : '') + '" id="potates-breakdown"><div style="margin-top:10px;padding-right:12px;text-align:left">' +
    bdSection('https://raw.githubusercontent.com/nachostax/la-salud2/main/FOOD.png','Food') +
    ...
    bdSection('https://raw.githubusercontent.com/nachostax/la-salud2/main/MOBILITY.png','Movement') +
    ...
    bdr('HIIT', bd.moveDetail.hiitPts, 5, bd.moveData.hiitSessionsAchieved > 0 ? bd.moveData.hiitSessionsAchieved + ' session' + (bd.moveData.hiitSessionsAchieved>1?'s':'') + ' this period' : 'no data') +
    ...
    bdSection('https://raw.githubusercontent.com/nachostax/la-salud2/main/WATER2.png','Water') +
    bdr('Water', bd.waterPts, 10, bd.hasData.water ? bd.waterHitDays + ' of ' + bd.waterTotalDays + ' days hit goal' : 'no data') +
    bdSection('https://raw.githubusercontent.com/nachostax/la-salud2/main/LIST.png','Logging') +
  ...
  return `<div class="day-card potates-score-card" style="margin-bottom:10px">
    <div class="day-card-body" style="text-align:center;padding:10px 14px 8px">
      <div style="position:relative">
        <div class="box-title" style="text-align:left">Potates Score</div>
        <div class="score-status-label" style="font-family:'Baloo 2',sans-serif;font-size:18px;letter-spacing:3px;color:${scoreColor};opacity:0.85;margin-bottom:4px;text-transform:uppercase;transition:color .4s;text-align:left">${statusLabel}</div>
        <div class="big-kcal" style="position:absolute;top:50%;right:30%;transform:translate(50%,-50%);font-size:32px;margin:0"><span id="potates-score-num" data-target-num="${score}" style="color:${numColor};filter:saturate(.55) brightness(1.2);transition:color .4s">0</span><span style="font-size:14px;color:var(--mist)">/100</span></div>
      </div>
