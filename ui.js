...
(function() {
  ...
  // Keyed per-person, not global: Nacho's phone and Gabi's phone are each
  // parked on their own person, so their heartbeats must never collide.
  // This now only locks a SECOND tab open on the SAME person (e.g. two
  // browser tabs on one phone) — it no longer blocks Nacho and Gabi working
  // out at the same time on their own devices.
  function hbKey() { return 'la-salud-tab-hb-' + (S.currentPerson || 'gabi'); }
  ...
  function writeHeartbeat() {
    try { localStorage.setItem(hbKey(), JSON.stringify({ id: TAB_ID, ts: Date.now() })); } catch(e) {}
  }

  function checkForOtherTab() {
    try {
      const raw = localStorage.getItem(hbKey());
  ...
  // StorageEvent fires in OTHER tabs when localStorage changes — instant
  // unlock. Prefix-matched since the key now varies by current person.
  window.addEventListener('storage', e => {
    if (e.key && e.key.indexOf('la-salud-tab-hb-') === 0) checkForOtherTab();
  });
...
const SUBSEC_TITLES = { settings:'Settings', targets:'Profile & Targets', history:'History', apikey:'API Key & Sync', notifications:'Notifications', quicklog:'Quick Log Edits' };
...
// ── ADAPTIVE ENGINE (Phase 2 — unified "verified days" confidence model) ───
// Replaces old 2.6/2.7. Trend confidence is driven by how many days inside
// the weigh-in window are ALSO marked fully-logged (full_day on meal
// entries) — not raw calendar span, not raw point-count alone. This is what
// lets a short, fully-logged window (a "perfect week") react fast even
// though it's sparse by old span-based rules, while a long window with no
// logged evidence (e.g. 2 months, 0 logged days) barely moves anything even
// with the same point count. See trail doc Phase 2 section for the two
// worked examples this was built against.
//
...
function computeTrendConfidence(person) {
  ...
  const totalSpanDays = Math.max(1, daysBetween(windowStart, windowEnd) + 1);

  // Verified days: dates inside [windowStart, windowEnd] with a fully-logged
  // meal entry for this person (full_day flag — same flag Progress/Potates
  // Score already use for logging-quality).
  ...
  // Regression: x = day-index from windowStart, y = kg.
...
// Now ALSO runs for recomposition goals: the intended rate there is fixed
// at 0 kg/week (hold steady while training reshapes composition) instead of
// coming from calc.appliedRateKgWeek/isLoss, which calculateDailyTargetFrom
// returns as null for recomposition since there's no formula-derived rate.
function maybeRunAdaptiveRecalc(person) {
  ...
  if (daysSinceLast < cadenceDays) return; // not due yet

  const calc = calculateDailyTargetFrom(m, person);

  let intendedRateKgWeek;
  if (m.goalType === 'recomposition') {
    intendedRateKgWeek = 0; // hold steady — recomp has no formula-derived rate to read
  } else {
    if (calc.appliedRateKgWeek == null) return; // shouldn't happen outside recomposition, guarding anyway
    intendedRateKgWeek = calc.isLoss ? -calc.appliedRateKgWeek : calc.appliedRateKgWeek;
  }
...
// goals never call this — 5.2's fixed-15%-deficit path bypasses it entirely.
...
function calculateDailyTargetFrom(m, person) {
  ...
  // RECOMPOSITION (Phase 5.2, updated): no target weight drives this goal —
  // the point is holding weight flat while shifting fat→muscle, so the
  // target is maintenance calories — TDEE, no deficit. (Phase 5.2 originally
  // applied a 15% below-TDEE deficit here, citing RP/3DMJ-style evidence-
  // based guidance that maintenance alone rarely produces visible recomp —
  // changed to 0% per explicit instruction to keep TDEE maintained.) This
  // bypasses goalTargetWeight/weightDiff/safetyClampDeficit entirely —
  // there is no weight-rate goal to clamp here, so goalRateClamped is
  // always false for this goal type.
  if (m.goalType === 'recomposition') {
    const RECOMP_DEFICIT_PCT = 0;
    const dailyDeficit = -Math.round(tdee * RECOMP_DEFICIT_PCT);
    ...
    return { bmr: Math.round(bmr), tdee: Math.round(tdee), dailyDeficit, target, activitySource, blendNote, clamped,
             goalTargetDate: targetDate, daysRemaining, goalRateClamped: false, goalSafeDaysNeeded: daysRemaining,
             recommendedRateKgWeek: null, maxRateKgWeek: null, appliedRateKgWeek: null, usedOverride: false, isLoss: null };
  }
...
async function calculateMyIntake(person) {
  ...
  // Macros are derived from the FINAL (possibly AI-blended) kcal target,
  // not the pre-blend formula target applyCalculatedTarget() used above.
  S.mission[person].kcal = finalTarget;
  const macros = calculateMacrosFrom(S.mission[person], finalTarget);
  S.mission[person].protein = macros.protein;
  S.mission[person].carbs = macros.carbs;
...
// reverts the input to the last valid value. Recomposition goals don't use
// this at all (no rate box shown for them — see settings.js).
function applyRateOverride(person, typedValue) {
  ...
  if (recommended == null) return; // recomposition goal, shouldn't reach here
...
function renderActivityControls(person) {
  ...
  if (calc.recommendedRateKgWeek != null) {
    // Not a recomposition goal — show and populate the rate box.
    ...
    // Recomposition goal — no rate concept, hide the box entirely.
